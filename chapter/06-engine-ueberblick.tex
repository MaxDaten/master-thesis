...
In \fref{sec:abstrakte-definition-haskell} wurde bereits erwähnt, dass sich für den Mealy-Typen nützliche Klassen-Instanzen implementieren lassen. Die konkreten Implementierungen für den Mealy-Typen wurden übersprungen aber in diesem Kapitel werden äquivalente Implementierungen für unseren Typen |RenderSystem| entwickelt. Aus Platzgründen werden nur die jeweils minimal notwendigen Definitionen angegeben, vollständige Definition finden sich im Modul |Yage.Rendering.RenderSystem| im Projekt.

Praktische Anwendungen werden in \fref{chap:anwendungsbeispiele} gegeben, doch eine umfassende Übersicht kann auf Grund des Umfangs nicht gegegeben werden. Umfassende Beispiele finden sich ebenso im Projekt beispielsweise im Modul \linebreak|Yage.Rendering.Pipeline.Deferred|.
