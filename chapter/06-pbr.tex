...
Der spekulare Beitrag $f_s$ wird durch die drei Faktoren bestimmt: Dem Fresnel $\mathcal{F}$ Term, der Microfacet Richtungsverteilung, $\mathbf D$ und der geometrischen Abschwächung (Geometrical Attenuation) $\mathbf G$.

\subsubsection[Fresnel]{Fresnel $\mathcal{F}$}
Der Fresnel Effekt beschreibt die Beobachtung, dass Oberflächen reflektierender erscheinen, umso flacher der Betrachtungswinkel wird. Als Approximation wurde die \textit{Schlick Approximation} \parencite{Schlick1994} gewählt. In \cite{Lagarde2012} wurde eine weitere Modifikation vorgeschlagen um die Berechnung zu vereinfachen, die hier ebenso wie in \cite[Seite 3]{Karis2013} angewendet wird.
...
$D$ beschreibt die statistische Normalverteilung der Ausrichtung der Microfacets entsprechend der Winkelhalbierenden $H$. Die Microfacets von glatten Oberflächen sind überwiegend gleich ausgerichtet, so dass Licht hauptsächlich entlang $R$ reflektiert wird. Rauen Oberflächen besitzen eher zufällig ausgerichtete Mircofacets, so dass Licht breiter gestreut wird. Die Rauheit der Oberfläche wird mit $m$ ausgedrückt. Zur Approximation von $D$ finden sich wiederum eine vielzahl von Modellen. Die Implementierung verwendet das als \textit{GGX} in \cite{Walter2007} vorgestellte Modell (siehe \fref{eq:ggx} nach \cite[Seite 3]{Karis2013}). $F_0$ beschreibt die Reflektanz für parallel zur Normale $\mathbf N$ einfallendes Licht.
...
$G$ beschreibt einen Faktor, der die Tatsache simuliert dass Microfacets sich gegenseitig das Licht blockieren und sich somit gegenseitig schattieren können. Es wurd die in \cite[Seite 3]{Karis2013}) modifizierte \textit{Schlick} Approximation übernommen \warn{Begründung?!}.
